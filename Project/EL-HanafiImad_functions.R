    ...
    #' returns the optimized parametres (Mu, Sigma, p) as a list 
{
    ...
    p <-  NK/N
    mustar <- 1/NK * t(respons)%*%x
    Sigstar=array(dim=c(d,d,k))
    for( i in (1:k)){
        mu_i = matrix(rep(mustar[i,],N),ncol = d,byrow = TRUE)
        #X_center = t(t(x) - mustar[i,] )
        Sigstar[,,i] <- 1/NK[i] * (t(respons[,i]*(x-mu_i))%*%((x-mu_i)))
    }
...
{ 
    ...
    {
        niter <- niter+1
        respons <- estep(x,current$Mu,current$Sigma,current$p)
            current <- mstep(x,respons)
    ...
    #' Beta: k-vector                   
{
    ...
    M <-  (beta0*m0 + NK*barx)/(Beta) ## a k*d matrix
    d <- ncol(x)
    ...
    for( j in (1:K)){
        ...
        x_center_j = t(t(barx[j,])-m0)
        Winv[,,j] <- W0inv + NK[j]*Sj + (beta0*NK[j])/(beta0+NK[j])*(x_center_j)%*%t(x_center_j)
    }
    ...
    #' returns: a n*k matrix: the responsibilities for each data point.  
{
    ...
    logResponsT <- -0.5*Equadratic + d/2*log(2*pi) + 0.5*Eloglambda + Elogrho
      ##  the transpose of
    ...
    
{
    ...
    for(j in (1:k))
    {
        Mu[j,] <- rmvn(n=1, Mu[j,], ppar$var_Mu*diag(1,d) )##use function rmvn 
        Sigma[,,j] <- rwishart(ppar$nu_Sigma,1/ppar$nu_Sigma * Sigma[,,j]) ##  use function rwishart
    }
    ...
    #' - naccept! number of accepted proposals. 
{
    ...
    for (niter in 1:nsample){
        ...
        lacceptratio <-  min((proposal$lpost+ddirichlet(current$p,alphaPropback, log=TRUE)+llkbackSigma -current$lpost -ddirichlet(proposal$p,alphaPropmove,log =TRUE) -llkmoveSigma),0)
            # pas de Normal parce que c'est symetrique...
    ...
    #' returns: a single numeric value
{
    ...
    vectllk <- vapply(inds, function(niter){
      exp(gmllk(x,sample$Mu[,,niter],sample$Sigma[,,,niter],sample$p[,niter]))},
      FUN.VALUE = numeric(1))
      return(mean(vectllk))
}
